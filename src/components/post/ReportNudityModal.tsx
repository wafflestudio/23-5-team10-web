...
interface ReportNudityModalProps {
  onClose: () => void
  onBack: () => void
  onNext: (step: 'age_check') => void
  onSubmit: (type: ReportType) => void
}

const NUDITY_REASONS = [
  '나체 이미지 또는 성적 행위',
  '성매매',
  '나체 이미지를 공유하거나 공유하겠다는 위협',
  '성인 나체 이미지',
  '성인 대상 성적 학대',
  '아동 나체 이미지 또는 아동 대상 성적 학대',
]

export default function ReportNudityModal({
  onClose,
  onBack,
  onNext,
  onSubmit,
}: ReportNudityModalProps) {
  return (
    <div
